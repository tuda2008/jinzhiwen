ActiveAdmin.register Category do
  permit_params :title, :intro, {images:[], supplier_ids:[], product_ids:[]}, :visible
  ...
  index do
    ...
    id_column
    column :title
    column :suppliers do |category|
      category.suppliers.visible.map(&:name).join('，')
    end
  ...
  form html: { multipart: true } do |f|
    ...
    f.inputs do
      f.input :title
      f.input :intro
      f.input :suppliers, :as => :check_boxes, :collection => Supplier.visible.pluck(:name, :id)
